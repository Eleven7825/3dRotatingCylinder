...
#include "RotatingCylinderKinematics.h"
...
//    d_a.resize(total_levels);
 //   d_Xc.resize(total_levels);
...
//	    d_a[d].resize(nodes_this_ln);
 //           d_Xc[d].resize(nodes_this_ln);
...
OscillatingCylinderKinematics::setOscillatingCylinderSpecificVelocity(const double time)
{
    ...
    //pout<< "Getting pointer for Xdata...";
    //Pointer<LData> X_data = d_l_data_manager->getLData("X", d_finest_ln);
    //pout<<"Done!\n";
    //getShape(0);
    ...
    const std::vector<double>& tagged_pt_position)
{
    ...
    pout<< "incremented_angle_from_reference_axis:	" <<incremented_angle_from_reference_axis[0]<<"\t"<<incremented_angle_from_reference_axis[1]<<"\t"<<incremented_angle_from_reference_axis[2]<<"\t"<<"\n";
    pout<< "center_of_mass:	"<<center_of_mass[0]<<"\t"<<center_of_mass[1]<<"\t"<<center_of_mass[2]<<"\t"<<"\n"; 
    
pout<<"tagged_pt_position:	"<<tagged_pt_position[0]<<"\t"<<tagged_pt_position[1]<<"\t"<<tagged_pt_position[2]<<"\n";
